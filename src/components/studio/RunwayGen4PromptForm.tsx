...
const helper: Record<string, string> = {
  ...
  Duration: "Total seconds (1-4)",
  Seed: "Fixed number for reproducible output",
  "Style Preset": "Overall aesthetic treatment",
  "Style Strength": "How intensely the preset is applied",
  Pan: "Horizontal camera move",
  Tilt: "Vertical camera move",
  Roll: "Rotate camera (Dutch angle)",
  Zoom: "Move in or out",
};
...
export default function RunwayGen4PromptForm({ onPromptGenerated }: RunwayPromptFormProps) {
  ...
  // Logic to build the final prompt string
  useEffect(() => {
    const motion = [
      pan !== "None" && `Pan ${pan}`,
      tilt !== "None" && `Tilt ${tilt}`,
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-end gap-2">
        <Switch checked={showHints} onCheckedChange={setShowHints} id="hints-toggle" />
        <Label htmlFor="hints-toggle" className="text-sm">Show hints</Label>
      </div>

      <TextAreaWithIcon
        label="Prompt"
        placeholder="e.g. Impressionist painting of a bustling Paris street coming to life"
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        onBullseye={() => console.log("Enhance prompt")}
      />
      <p className="text-xs text-muted-foreground -mt-4">
        Click the <Target className="w-3 h-3 inline stroke-red-600" /> to generate 3 AI-tuned prompt variants.
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="flex items-center gap-2 pt-6">
          <Checkbox id="upscale" checked={upscale} onCheckedChange={(checked) => setUpscale(Boolean(checked))} />
          <Label htmlFor="upscale">Upscale (4K)</Label>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {Dropdown("Pan", pan, setPan, panOptions)}
        {Dropdown("Tilt", tilt, setTilt, tiltOptions)}
        {Dropdown("Roll", roll, setRoll, rollOptions)}
        {Dropdown("Zoom", zoom, setZoom, zoomOptions)}
      </div>

      <Button className="w-full py-6 mt-4" onClick={() => { /* This button could trigger a final action if needed */ }}>
        Generate Prompt
      </Button>
    </section>
  );
