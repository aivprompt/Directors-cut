...
export default function Veo3PromptForm({ onPromptGenerated = () => {} }) {
  ...
  const handleEnhance = async (fieldType) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/generate-variants.cjs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ inputText }) });
  ...
  const handleGenerateClick = async () => {
    ...
    try {
      const response = await fetch('/api/generate-prompt.cjs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || 'API error');
